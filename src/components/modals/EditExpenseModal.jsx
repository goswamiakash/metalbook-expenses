...
const Modal = styled.div`background:var(--panel); padding:18px; border-radius:10px; width:380px; border:1px solid rgba(255,255,255,0.04);`;
...
export default function EditExpenseModal({ item, onClose }) {
  ...
  function submit(e){
    e.preventDefault();
    const payload = { id: form.id, name: form.name, amount: Number(form.amount), category: form.category, date: new Date(form.date).toISOString() };
